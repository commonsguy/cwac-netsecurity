 ...
 */
package com.commonsware.cwac.netseccfg.conscrypt;
import com.commonsware.cwac.netseccfg.luni.X509ExtendedTrustManager;
...
*/
  private List<X509Certificate> checkTrusted(X509Certificate[] chain, String authType,
    ...
    if (session != null && parameters != null) {
      String identificationAlgorithm = null; // parameters.getEndpointIdentificationAlgorithm();

      /* MLM reflection to try to get working on older Androids */

      try {
        Method method=parameters.getClass().getMethod("getEndpointIdentificationAlgorithm");

        if (method!=null) {
          session=(SSLSession)method.invoke(parameters);
        }
      }
      catch (Exception e) {
        android.util.Log.d("TrustManagerImpl", "Exception getting endpoint identification algorithm", e);
      }
